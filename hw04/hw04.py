...
def multiple_references_explanation():
    return """The multiple reference problem...  too hard to understand, so I pasted the answer here
    Eva is right. Because when an uncertain number is used in computation, the interval span will extend 
    no matter which arithmetic is used(sub, add, mul, div). The less these number is used, 
    the tighter the error bounds would be.
...
def quadratic(x, a, b, c):
    ...
    '0 to 10'
    """
    extreme_x = -b / (2 * a) # f'(t) = 0
    quadratic_fun = lambda t: a * t * t + b * t + c
    edge_y1, edge_y2, extreme_y = quadratic_fun(lower_bound(x)), quadratic_fun(upper_bound(x)), quadratic_fun(extreme_x)
    if lower_bound(x) < extreme_x < upper_bound(x):
        lower = min(edge_y1, edge_y2, extreme_y)
        upper = max(edge_y1, edge_y2, extreme_y)
    else:
        lower = min(edge_y1, edge_y2)
        upper = max(edge_y1, edge_y2)
    return interval(lower, upper)
